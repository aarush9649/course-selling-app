...
function Buy() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      fetchBuyCourseData();  // Fetch course data if token is available
    }
  }, [token, navigate]); // This effect depends on the token and navigate function

  const fetchBuyCourseData = async () => {
    try {
      const response = await axios.post(
        `http://localhost:4001/api/v1/courses/buy/${courseId}`,
        {},
        {
          headers: {
