  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4001/api/v1/user/login", // Assuming this is the login endpoint
        { email, password },
        {
      ...
      <div className="h-screen container mx-auto flex  items-center justify-center text-white">
        ...
        <header className="absolute top-0 left-0 w-full flex justify-between items-center p-5  ">
          ...
          <div className="flex items-center space-x-4">
            <Link
              to={"/signup"}
              className="bg-transparent border text-blue-700 border-gray-500 p-1 text-sm md:text-md md:py-2 md:px-4 rounded-md"
            >
              Signup
            </Link>
            <Link
        ...
        <div className="bg-gray-900 p-8 rounded-lg shadow-lg w-[500px] m-8 md:m-0 mt-20">
          <h2 className="text-2xl font-bold mb-4 text-center">
            Welcome to <span className="text-orange-500">SmartLearn</span>
          </h2>
