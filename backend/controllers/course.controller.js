...
if(!req.files || Object.keys(req.files).length === 0){
  return res.status(400).json({ errors: "No image provided" });
 
}
const allowedFormat = ['image/jpeg', 'image/png'];

if (!allowedFormat.includes(image.mimetype)) {
  return res.status(400).json({ errors: "Invalid image type. Only JPEG and PNG are allowed." });
}
...
export const getCourses=async(req,res)=>{
  try{
    ...
    console.log("Error in course fetching courses",error);
  }
}
